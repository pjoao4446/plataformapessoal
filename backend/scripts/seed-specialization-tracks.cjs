    ...
    for (const track of awsTracks) {
      try {
        const stmt = db.prepare(`
          INSERT INTO courses (title, subtitle, description, type, segment, level, image_path, created_at) 
          VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
        `);
        const result = stmt.run(track.title, track.subtitle, track.description, track.type, track.segment, track.level, track.image_path);
